...
def oversample_data(train_features_path, train_labels_path, output_dir):
    ...
    pd.DataFrame(X_train_smote, columns=feature_names).to_csv(f"{output_dir}/train_features_smote.csv", index=False)
    pd.DataFrame(y_train_smote, columns=[y_train.name]).to_csv(f"{output_dir}/train_labels_smote.csv", index=False)
    
    # Save the scaler model for later use
    scaler_model_path = f"{output_dir}/scaler_model.joblib"
